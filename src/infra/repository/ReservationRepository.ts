...
export class ReservationRepositoryDatabase implements ReservationRepository {
	...
	async hasActiveReservations(roomId: string, checkinDate: Date, checkoutDate: Date): Promise<boolean> {
		...
		const reservations = await connection.query("select * from reservation where room_id = $1 and (checkin_date, checkout_date) overlaps ($2, $3) and status = 'active'", [roomId, checkinDate, checkoutDate]);
